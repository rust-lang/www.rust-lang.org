...
networking-production-testimonial-buoyant = Rust является основой технологической дорожной карты Linkerd. Его система типов позволяет нам содавать модульные, тестируемые компонуемые единицы без ущерба для производительности во время исполнения. Однако самое удивительное то, что система проверки заимствований и времён жизни, позволяет нам избежать больших утечек ресурсов. После 2 лет, я не могут представить использование для работы какого-либо другого языка.
