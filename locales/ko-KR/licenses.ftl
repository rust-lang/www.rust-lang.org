### Translation file for page: https://www.rust-lang.org/policies/licenses
## templates/policies/licenses.hbs

policies-licenses-page-title = Licenses

licenses-license-heading = 라이센스
licenses-license-general-description =
        "러스트 프로그래밍 언어"와 이 웹사이트를 포함한 모든 공식 프로젝트는 일반적으로 이중 라이센스입니다.
licenses-apache-v-2-0-link = Apache License, Version 2.0
licenses-mit-link = MIT license
licenses-license-specific-description =
        <p>각 프로젝트의 특정 라이센스 정보 깃허브 리포지토리에서 확인할 수 있습니다.</p>
        <p>제 3자 로고는 제 3자 저작권 및 상표의 적용을 받을 수 있으며, 나머지 러스트 웹 사이트와 동일한 라이센스로 사용할 수 없습니다.</p>
        <p>러스트 프로젝트 또는 관련 프로젝트에 대한 라이센스 부여 방법에 대한 구체적인 질문이 있는 경우 러스트 코어 팀에 문의하십시오.</p>
licenses-license-email-link = 코어 팀에 메일 보내기

licenses-attribution-heading = 기여
licenses-attribution-description =
        아이콘은 <a href="https://www.flaticon.com/" title="Flaticon">www.flaticon.com</a>의 <a href="http://www.freepik.com" title="Freepik">Freepik</a>에 의해 제작되었으며, <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0">CC-3.0-BY</a>에서 부여된 라이센스입니다.
