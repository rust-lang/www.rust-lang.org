...
embedded-pitch-analysis-description = 在编译时强制要求引脚和外设配置，以确保资源不会被您应用程序中非预期的部分使用。
embedded-pitch-memory-alt = 内存条
embedded-pitch-memory-heading = 灵活的内存管理
embedded-pitch-memory-description = 动态内存分配是可选的。可使用全局分配器和动态数据结构，也可以不采用堆而全部静态分配。
embedded-pitch-concurrency-alt = 齿轮
embedded-pitch-concurrency-heading = 无畏并发
embedded-pitch-concurrency-description = Rust 使得线程之间不可能意外地共享状态。您既能使用喜欢的任意并发方式，同时又能获得 Rust 提供的强力保障。
embedded-pitch-interop-alt = 握手
embedded-pitch-interop-heading = 互操作性
embedded-pitch-interop-description = 可将Rust集成到现有的 C 代码库中，也可利用现有的 SDK 编写 Rust 应用程序。
embedded-pitch-portability-alt = 行李推车
embedded-pitch-portability-heading = 可移植性
embedded-pitch-portability-description = 只需要一次编写库或驱动，就能在各种系统中使用它。不管是非常小的微控制器，还是强大的单板计算机，都可直接使用。
embedded-pitch-community-alt = 盾牌标志
embedded-pitch-community-heading = 社区驱动
embedded-pitch-community-description = 作为 Rust 开源项目的一部分，嵌入式系统由一流的并拥有商业伙伴支持的开源社区驱动。
...
embedded-get-started-discovery-book-description = 从零开始学习嵌入式开发 &mdash; 使用 Rust 语言！
embedded-get-started-embedded-rust-book-alt = QFP-20 包
embedded-get-started-embedded-rust-book-heading = 《嵌入式 Rust 之书》
embedded-get-started-embedded-rust-book-description = 已经对嵌入式开发很熟悉了？立刻上手 Rust 来开始获取更多优势。
embedded-get-started-embedonomicon-alt = BGA 包
embedded-get-started-embedonomicon-heading = 《嵌入式秘典》
embedded-get-started-embedonomicon-description = 探究基础性嵌入式开发库的工作原理。
...
embedded-testimonials-sensirion-quote = 在 Sensirion，我们最近用 Rust 为本公司出品的 <a href="https://sensirion-automotive.com/products#PM2_5">颗粒物质传感器</a>构建了嵌入式样品机。由于交叉编译的简便和 crates.io 上的众多高品质依赖包的存在，<b>我们迅速地完成了既快速又健壮的样品机。</b>
embedded-testimonials-sensirion-attribution = &ndash; Raphael Nestler, 软件工程师, Sensirion
embedded-testimonials-airborne-alt = Airborne Engineering Ltd 标志
embedded-testimonials-airborne-quote = 在 Airborne Engineering Ltd，我们最近用 Rust 语言写了以太网启动加载器，<a href="https://github.com/airborneengineering/blethrs">blethrs</a>，用于内部数据采集系统。<b>Rust 是一门有前途的语言，我们对于将它用于未来无论是嵌入式还是其他类的项目，都感到兴奋。</b>
embedded-testimonials-airborne-attribution = &ndash; Dr. Adam Greig, 仪器工程师, Airborne Engineering Ltd.
...
embedded-testimonials-49nord-quote = { "[" }Rust] 使得我们可以比想象中更快、更健壮的软件推向市场。感谢 Rust，让我们将内存安全看得理所当然，同时带有复杂类型系统的零开销语言所带来的其他好处，帮助我们开发出了可维护的软件。<b>Rust 既让我们的客户变得快乐，也让我们的工程师变得快乐。</b>
...
embedded-testimonials-terminal-tech-quote = 以往，在嵌入式领域被认为 C/C++ 是不可替代的选择。<b>但现在，我们认为在嵌入式领域能够使用一门现代化的优秀语言真的很棒。</b>
embedded-testimonials-terminal-tech-attribution = &ndash; Aleksei Arbuzov, 高级软件工程师, Terminal Technologies
