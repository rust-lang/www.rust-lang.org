...
wasm-js-augment-description = WebAssembly 的梦想不是扼杀 JavaScript，而是与其并存，来帮助处理繁重或底层的一些任务。Rust以性能为先，这些任务也可以受益。
...
wasm-production-cloudflare-quote = 我们可将 Rust 编译成 WASM，并在编织到互联网结构中的 Serverless 功能中调用它。它的潜力巨大，我迫不及待地想要用它做更多的事情。
...
wasm-production-mozilla-quote = { "[" }<code>source-map</code> 库的] JavaScript 实现以性能为由累积了大量的复杂代码，而我们将它替换为地道的 Rust。Rust 不会强迫我们在明确地表达意图和运行时性能之间做出选择。
...
# If an opening square bracket is the first character on a line, it must be escaped like this: {"["}. See https://projectfluent.org/fluent/guide/special.html
wasm-production-dropbox-quote = { "[" }Rust 的]特性使然，通过 WASM 可以轻松地将 DivANS 编解码器嵌入到网页中，如前文所述。
