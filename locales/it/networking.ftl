...
networking-pitch-footprint = Tieni sotto controllo le risorse di sistema per tenere al minimo l'utilizzo di memoria e CPU. Fatti aiutare dal compilatore per scrivere codice corretto. Il tutto in un ecosistema produttivo e piacevole da usare.
networking-pitch-footprint-alt = Una piuma
networking-pitch-secure-header = Sicuro e affidabile
networking-pitch-secure = Il potente type checker di Rust previene intere classi di bug. Hai la certezza di sapere esattamente dove e quando un certo stato è condiviso o modificato. Fatti aiutare a evidenziare possibili punti deboli - prima del deploy.
networking-pitch-secure-alt = Uno scudo
networking-pitch-concurrent-header = Concorrenza e scalabilità
networking-pitch-concurrent = Usa l'approccio alla programmazione concorrente che si adatta meglio alle esigenze. Rust previene che thread o task concorrenti condividano gli stati. Rust ti permette di scalare fin dove possibile, senza alcuna preoccupazione.
...
networking-production-testimonial-1aim = Il potente type system di Rust permette di rifattorare il codice in sicurezza e di catturare numerose classi di errori già in fase di compilazione. Il basso consumo di risorse e le alte prestazioni, insieme alla sicurezza nella gestione della memoria e a un solido sistema di gestione errori, fanno di Rust lo strumento adatto per avere nel nostro backend, in ambiti ove la sicurezza è tutto, complesse logiche business. L'altissima stabilità significa che la nostra applicazione in Rust sia l'ultimo componente del nostro stack tecnologico di cui doverci preoccupare. Abbiamo in produzione un backend al 100% in Rust dalla fine del 2015 e finora non ci ha mai dato problemi!
