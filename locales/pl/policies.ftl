### Translation file for page: https://www.rust-lang.org/policies/


## templates/policies/index.hbs

policies-page-title = Zasady
policies-page-description = Ta strona zawiera linki do kompletnego spisu zasad obowiązujących projekt Rust.
policies-code-of-conduct-link = Kodeks Postępowania
policies-licenses-link = Licencje
policies-security-link = Notki dot. Bezpieczeństwa
policies-privacy-link = Polityka Prywatności
policies-reach-out-description = Nie możesz czegoś znaleźć? Masz pytanie? Skontaktuj się z nami!
policies-reach-out-link = Napisz do Głównego Zespołu
policies-translation-warning =
    Umożliwiamy członkom społeczności Rusta tłumaczenie niniejszych zasad na 
    inne języki. W przypadku jakichkolwiek niezgodności pomiędzy tłumaczeniem
    a <a href="{ $english }">wersją w języku angielskim</a>, za obowiązującą
    uważa się wersję angielską.
