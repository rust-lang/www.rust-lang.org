...
learn-doc = 這些文件您都能在本地端使用 <code>rustup doc</code> 指令來取得，它能在沒有網路連線的狀況下使用您的瀏覽器開啟這些文件！
...
learn-domain-embedded = 精通使用 Rust 在微處理器或其它嵌入式系統上
...
learn-install-fmt-clippy = 您可以用 <code>rustup component add rustfmt</code> 安裝程式碼格式化工具（Rustfmt），以及用 <code>rustup component add clippy</code> 安裝 lint 工具（Clippy）。
...
learn-ferris =
    <p>Ferris 是隻Rust 社群非官方的吉祥物，許多 Rust 程式設計師會自稱「Rustaceans」，因為這和「<a href="https://en.wikipedia.org/wiki/Crustacean">crustacean</a>」相似。我們指名 Ferris 會用代名詞「they」、「them」等等，而不是帶有性別的代名詞。</p>
    <p>Ferris 的名字近似於形容詞「ferrous」，含意與鐵有關。既然 Rust（銹）通常來自於鐵，我們認為這是個有趣的吉祥物名稱源由！</p>
    <p>您可以在 <a href="http://rustacean.net/">http://rustacean.net/</a> 找到更多 Ferris 的圖片。
